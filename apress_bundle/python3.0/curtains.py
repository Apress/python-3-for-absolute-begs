...
# We will assume that the roll of material is going to be 140cm
# and that the price per metre will be 5 units of currency
roll_width = 140
price_per_metre = 5

# Prompt the user to input the window measurements in cm
window_height = eval(input('Enter the height of the window (cm): '))
window_width = eval(input('Enter the width of the window (cm): '))

# print headers for our rather basic trace table
print() 
print('\twidth\theight\twidths\ttotal\tprice\tshorter?\twider?')

# We need to add a bit for the hems
# First we must convert the raw string into a float
# otherwise we will get an error if we try to perform arithmetic on a 
# text string
...
# Now we need to work out how many widths of cloth will be needed
# and figure out the total length of material for each curtain (in cm still)
# If the length of the curtains is less than the roll_width, we can turn the
# whole thing on its side and just use one width of fabric, but if the curtains
# need to be both longer and wider than the roll_width, then we have a further
# problem - if the extra material required is less than half the roll_width we
# would need to buy an additional width of material at the same length; if it is
# more than half, then we would need to buy two additional widths.
...
# Finally we need to work out how much it will cost
